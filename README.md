...
> - **newcamd descrambler client** — connects to a newcamd-compatible cardserver as a card-emulation client, as an alternative to the dvbapi socket. One TCP connection per CAID, learned at login. Enable with `-DNEWCAMD=ON` and use `--newcamd host:port:user:pass:deskey`. The flag is repeatable, and a single value may carry several comma-separated endpoints. To keep credentials off the process command line (visible via `ps`) put them in the `MINISAT_NEWCAMD` env var instead — minisatip reads it at startup using the same syntax:
>
>   ```sh
